...
//    cAddressFile,cMemoryFile: TCompressionStream;
  ...
  private
    ...
    asioHandled: boolean; //true when asio R0 engine handled the scan
    ...
    function AsioFirstScan: boolean; //delegate first scan to asio R0 engine
    function AsioNextScan: boolean;  //delegate next scan to asio R0 engine
  public
  ...
  private
    ...
    lastScanUsedAsio: boolean; //true if last scan was delegated to asio R0
    {$IFNDEF lowmemoryusage}
...
//===============Local functions================//
function ParseAOBString(const st: string; var pattern, mask: SysUtils.TBytes): boolean;
{
Parses an AOB string like "AA BB CC ?? DD" into pattern and mask arrays.
?? or * = wildcard (mask byte = 0x00), exact hex values get mask byte = 0xFF.
...
begin
  result := false;
  ConvertStringToBytes(st, true, bytes, false);
  n := length(bytes);
  if n = 0 then exit;
...
function TScanController.AsioFirstScan: boolean;
{
Delegate first scan to asio R0 engine. Returns true if handled.
Supports: byte/word/dword/qword/float/double with exact/between/bigger/smaller/changed/unchanged.
Also supports AOB (vtByteArray) with exact match.
}
var
  vt, so: uint8;
  vlo, vhi: uint64;
  hits: AsioBridge.TUint64Array;
  scanValueU64: uint64;
  scanValueSingle: single;
  scanValueDouble: double;
  i: integer;
  datatype: string[6];
  af: TFileStream;
  addr: ptruint;
  pattern, mask: SysUtils.TBytes;
  patLen: integer;
begin
  result := false;
  {$ifdef windows}
  if not AsioReady then exit;
  if scanType <> stFirstScan then exit;
  if scanOption = soForgot then exit; // unknown value not supported via asio

  // Map variable type to asio value type
  case variableType of
    vtByte:     vt := ASIO_SCAN_T_BYTE;
    vtWord:     vt := ASIO_SCAN_T_WORD;
    vtDWord:    vt := ASIO_SCAN_T_DWORD;
    vtQWord:    vt := ASIO_SCAN_T_QWORD;
    vtSingle:   vt := ASIO_SCAN_T_FLOAT;
    vtDouble:   vt := ASIO_SCAN_T_DOUBLE;
    vtByteArray:
    begin
      // AOB scan — parse pattern and mask from scanvalue1
      // scanvalue1 format: "AA BB CC ?? DD" where ?? = wildcard
      if not ParseAOBString(scanvalue1, pattern, mask) then exit;
      patLen := length(pattern);
      if patLen = 0 then exit;

      if not AsioScanAob(startaddress, stopaddress, fastscanalignment,
                         pattern, mask, hits) then exit;

      // Write results to ADDRESSES.TMP
      datatype := 'NORMAL';
      af := TFileStream.Create(OwningMemScan.ScanresultFolder + 'ADDRESSES.TMP', fmCreate);
      try
        af.WriteBuffer(datatype, sizeof(datatype));
        for i := 0 to high(hits) do
        begin
          addr := hits[i];
          af.WriteBuffer(addr, sizeof(ptruint));
        end;
      finally
        af.Free;
      end;

      OwningMemScan.found := length(hits);
      result := true;
      exit;
    end;
  else
    exit; // unsupported type (grouped, binary, custom, string)
  end;

  // Map scan option to asio scan op
  case scanOption of
    soExactValue:       so := ASIO_SCAN_OP_EQ;
    soValueBetween:     so := ASIO_SCAN_OP_RANGE;
    soBiggerThan:       so := ASIO_SCAN_OP_GT;
    soSmallerThan:      so := ASIO_SCAN_OP_LT;
    soIncreasedValue:   so := ASIO_SCAN_OP_INCREASED;
    soDecreasedValue:   so := ASIO_SCAN_OP_DECREASED;
    soChanged:          so := ASIO_SCAN_OP_CHANGED;
    soUnChanged:        so := ASIO_SCAN_OP_UNCHANGED;
  else
    exit; // percentage, luaformula, etc. not supported via asio
  end;
  ...
  begin
    case variableType of
      ...
      begin
        if hexadecimal then
        begin
          vlo := StrToInt64('$' + scanvalue1);
          vhi := StrToInt64('$' + scanvalue2);
        end
        else
        begin
          vlo := StrToInt64(scanvalue1);
      ...
      vtSingle:
      begin
        scanValueSingle := StrToFloat(scanvalue1);
        Move(scanValueSingle, vlo, 4);
        scanValueSingle := StrToFloat(scanvalue2);
        Move(scanValueSingle, vhi, 4);
      end;
      vtDouble:
      begin
        scanValueDouble := StrToFloat(scanvalue1);
        Move(scanValueDouble, vlo, 8);
        scanValueDouble := StrToFloat(scanvalue2);
        Move(scanValueDouble, vhi, 8);
      end;
    end;
  end
  else
  ...
  // Call asio scan
  if not AsioScanValue(startaddress, stopaddress, fastscanalignment,
                       vt, so, vlo, vhi, hits) then exit;
...
function TScanController.AsioNextScan: boolean;
{
Delegate next scan to asio R0 engine. Returns true if handled.
Uses ASIO_OP_SCAN_NEXT which operates on the server-side cached result list.
Supports the same scan options as AsioFirstScan.
}
var
  so: uint8;
  vlo, vhi: uint64;
...
begin
  ...
  {$ifdef windows}
  if not AsioReady then exit;
  if scanType <> stNextScan then exit;
  if not OwningMemScan.lastScanUsedAsio then exit; // server has no cached results
  ...
  case scanOption of
    ...
    soChanged:          so := ASIO_SCAN_OP_CHANGED;
    soUnChanged:        so := ASIO_SCAN_OP_UNCHANGED;
    soIncreasedValueBy: so := ASIO_SCAN_OP_INCREASED;
    soDecreasedValueBy: so := ASIO_SCAN_OP_DECREASED;
  else
    exit;
  end;
  ...
  else if scanOption in [soIncreasedValueBy, soDecreasedValueBy] then
  begin
    // For "changed by X" scans, parse as signed integer for proper delta handling
    case variableType of
      vtByte, vtWord, vtDWord, vtQWord:
      begin
        if hexadecimal then
          ...
          vlo := StrToInt64(scanvalue1);
        // Sign-extend delta values (deltas are inherently signed)
        case variableType of
          vtByte:  vlo := uint64(int64(shortint(byte(vlo))));
          vtWord:  vlo := uint64(int64(smallint(word(vlo))));
          vtDWord: vlo := uint64(int64(longint(dword(vlo))));
        end;
      end;
      vtSingle:
      begin
  ...
  // Call asio next scan
  if not AsioScanNext(so, vlo, vhi, hits) then exit;
 ...
 // OutputDebugString('TScanController.execute');
  ...
  try
    ...
    try
      ...
      // Try asio R0 delegation first (stealth: bypasses all usermode scanning)
      if (scantype=stFirstScan) and AsioFirstScan then
      begin
        asioHandled := true;
        OwningMemScan.lastScanUsedAsio := true;
      end
      else if (scantype=stNextScan) and AsioNextScan then
      begin
        asioHandled := true;
        // lastScanUsedAsio stays true
      end
      else
      begin
        // Fall back to normal CE scanning
        if scantype=stFirstScan then
        begin
          firstscan;
          OwningMemScan.lastScanUsedAsio := false;
        end;
  ...
  //  OutputDebugString('It actually finished');
    ...
    if (not asioHandled) and (scanOption<>soUnknownValue) then
    begin
      try
