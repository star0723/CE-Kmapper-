...
type
  ...
  TProcessWindow = class(TForm)
    ...
    procedure btnInputPIDClick(Sender: TObject);
...
uses MainUnit, formsettingsunit, advancedoptionsunit,frmProcessWatcherUnit,
  ...
  GDBServerDebuggerInterface, plugin, AsioBridge;
...
procedure TProcessWindow.btnInputPIDClick(Sender: TObject);
begin
  InputPIDmanually1Click(Sender);
end;
  ...
  try
    ...
    case TabHeader.TabIndex of
      ...
      begin
        if AsioReady then
        begin
          // R0 process enumeration via asio pipe — bypasses anti-cheat usermode hooks
          processlist.Items.Clear;
          if AsioEnumProcesses(procs) then
          begin
            for j := 0 to high(procs) do
              processlist.Items.Add(format('%.8x-%s', [procs[j].pid, TrimRight(procs[j].name)]));
          end
          else
            processlist.Items.Add('(R0 enum failed: ' + AsioGetLastError + ')');
        end
        else
