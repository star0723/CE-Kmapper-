...
begin
  ...
  begin
    if deletedService then
      messagebox(0,'The driver is successfully unloaded.','mtkdrv.sys unloaded',MB_ICONINFORMATION or MB_OK)
    else
      messagebox(0,'The driver failed to unload or is already unloaded. If you think it''s still loaded then reboot and run the unloader again.','mtkdrv.sys unloader',MB_ICONERROR or MB_OK)
  end;
end.
