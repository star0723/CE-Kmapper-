...
uses
  ...
  forms,dialogs, controls, maps, globals, AsioBridge;
...
begin
  ...
  // asio path: CR3 direct read, no syscall
  if AsioReady then
  begin
    result := AsioRead(lpBaseAddress, lpBuffer, nSize);
    if result then NumberOfBytesRead := nSize else NumberOfBytesRead := 0;
    exit;
  end;
...
begin
  ...
  begin
    ...
    // asio path: pseudo handle from AsioAttach
    if AsioReady then
      exit(ReadProcessMemory64_Internal(l.processid,lpBaseAddress, lpBuffer, nSize,NumberOfBytesRead));
...
begin
  ...
  // asio path: CR3 direct write, no syscall
  if AsioReady then
  begin
    result := AsioWrite(BaseAddress, lpBuffer, nSize);
    if result then NumberOfBytesWritten := nSize else NumberOfBytesWritten := 0;
    exit;
  end;
...
var output: record
    ...
    cc,x: dword;
    l: thandlelistEntry;
    cr3, imgBase, imgSize: QWord;
begin
  result:=0;
  if dwProcessId=0 then
    exit;

  // asio path: attach via R0 pipe, no OpenProcess syscall
  if AsioReady then
  begin
    ...
    begin
      ...
      try
        if handlemap.HasId(result) then handlemap.Delete(result);
        l.processid := dwProcessID;
        l.specialHandle := false;
        l.validhandle := false; // not a real OS handle
    ...
    // AsioAttach failed, fall through to original path
  end;
...
begin
  ...
  begin
    ...
    begin
      // Cache miss: launch async background load (non-blocking)
      AsioEnsureRegionCacheAsync;
      // Return MEM_FREE for now — cache will be ready on next query
...
begin
  ...
  // asio path: allocate via R0 pipe
  if AsioReady then
  begin
    if AsioAlloc(dwSize, flProtect, r) then
    begin
      result := pointer(r);
      exit;
    end;
